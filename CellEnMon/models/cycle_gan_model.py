...
class CycleGANModel(BaseModel):
    ...
    def backward_G(self):
        ...
        self.loss_cycle_B = 100 * torch.mean(L1(self.rec_B, self.real_B) * self.rain_rate_prob) #
        ...
        self.loss_G_B_only = 10 * torch.mean(L2(self.D_B, targets))

        # GAN loss D_A(G_B(B))
        self.D_A=self.netD_A(self.fake_A)
        targets = torch.full_like(self.D_A, 1.0).to(self.D_A.device)
        self.loss_G_A = 10 * torch.mean(L2(self.D_A, targets)) #weight=self.rr_norm.max(), weight=self.att_norm.mean()
