...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm1d, use_dropout=False, n_blocks=6, padding_type='zero'):
        ...
        model = [#nn.ReflectionPad2d(3),
                 nn.Conv1d(input_nc, ngf, kernel_size=7, padding_mode='reflect', bias=use_bias),
                 norm_layer(ngf),
                 nn.ReLU(True)]
        ...
        for i in range(n_downsampling):  # add upsampling layers
            ...
            model += [nn.ConvTranspose1d(ngf * mult, int(ngf * mult / 2),
                      ...
                      nn.ReLU(True)]
        model += [nn.ReflectionPad2d(3)]
        model += [nn.Conv1d(ngf, output_nc, kernel_size=7, padding_mode='reflect')]
        model += [nn.Tanh()]
